  ...
  return (
    <section ref={sectionRef} className="projects" id="projects">
      ...
      >
        <path
          ref={pathRef}
          d="M1925 132C1925 132 1268 -198 1282.95 209.648C1282.95 819.173 637.554 475.659 637.554 819.174C637.554 1162.69 1291.46 919.678 1291.46 1251.19C1291.46 1582.71 637.554 1398.7 637.554 1698.21C637.554 1997.72 1291.46 1873.72 1291.46 2119.23C1291.46 2364.74 725.174 2333.74 637.554 2333.74C549.934 2333.74 30.5424 2309 0 2208"
          stroke="#F97516"
          strokeWidth="8"
        />
        <g ref={arrowRef}>
          <polygon points="0,-20 40,0 0,20" fill="none" stroke="#F97516" strokeWidth="6" />
        </g>
      </svg>
      <FadeContent
        blur={true}
        ...
        {projects.map((project, index) => (
          ...
          >
            <div className="project-info">
              <Image alt={`Logo de ${project.title}`} src={project.logo} width={60} height={60} />
              <h3>{project.title}</h3>
              <p>{project.description}</p>
              <p>{project.stack}</p>
              <button className="project-link" onClick={() => openModal(project)}>
                En savoir plus
              </button>
            </div>
            <div className={`project-date ${index % 2 === 0 ? "right" : "left"}`}>
              {project.date}
            </div>
          </div>
        ))}
      </div>
      <ModalProjects open={modalOpen} project={selectedProject || projects[0]} onClose={closeModal} />
    </section>
  );
}